...
if(isset($_POST['forminscription']))
{
	...
	if(!empty($_POST['nom']) AND !empty($_POST['prenom']) AND !empty($_POST['mdp']) AND !empty($_POST['mdp2']) AND !empty($_POST['mail']))
	{
		    ...
		    if (preg_match("/^[a-zA-Z ]+[.]+[a-zA-Z]+@ynov[.]com$/",$mail)) 
		    {
				...
				if($mailexist == 0)
				{
					...
					if ($mdp == $mdp2)
					{
						...
						$insertmbr = $bdd->prepare("INSERT INTO users(nom, prenom, mdp, mail, rang) VALUES (?, ?, ?, ?, ?)");
						$insertmbr->execute(array($nom, $prenom, $mdp, $mail, $rang));


						$erreur = "Votre compte a bien été crée! Vous allez recevoir un mail de confirmation";
